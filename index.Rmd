...
df %>%
  nest(data = -c({{nest_vars}})) %>%
    mutate(freq =
           map(data, ~ tabyl(.x, {{outcome_var}}))) %>% # show_na = FALSEで欠損無しのpercentのみ算出
  select(-data) %>%
  unnest(cols = c(freq)) %>%
  as_tibble()
}
...
# 層：全体，island
quos(NULL, island) %>% 
  map( ~nest_tabyl(penguins, species, !!.x))
...
vars %>% 
  map( ~nest_tabyl(penguins, .x)) # !!.xでも動いた



# アウトカム：species, island；層：sex
quos(species, island) %>% 
  map( ~nest_tabyl(penguins, !!.x, sex))
...
ac_res <- 
map(ac, ~nest_tabyl(bfi, .x, gender)) %>% 
...
# アウトカム：cyl, gear；層：vs
quos(cyl, gear) %>% 
  map( ~nest_tabyl(mtcars, !!.x, vs))

# アウトカム：cyl, gear；層：vs x am
quos(cyl, gear) %>% 
  map( ~nest_tabyl(mtcars, !!.x, c(vs,am)))
...
map2(outcomes, groups,  ~nest_tabyl(mtcars, !!.x, !!.y))
```
...
res_l <- 
  outcomes %>% 
  str_c(., "~ species*sex") %>%
  map(~lm(as.formula(.x), data = penguins)) %>% 
  set_names(outcomes)
...
p1_2 %>%
  reduce(var_name, make_dif, .init = .) %>% 
  select(id, species, contains("bill_length"), contains("bill_depth")) %>% 
    glimpse()


p1_2 %>%
  reduce(var_name, make_dif2, .init = .) %>% 
  select(id, species, contains("bill_length"), contains("bill_depth")) %>% 
    glimpse()
...
df_multi <- 
  map(contents, ~multi_answer(df, .x)) |> 
  reduce(full_join)
